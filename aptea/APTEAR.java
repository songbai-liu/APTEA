...
public class APTEAR extends Algorithm{
	int populationSize;
	int maxEvaluations;
	int evaluations;
	...
	public SolutionSet execute() throws JMException, ClassNotFoundException {
		...
		while(evaluations <= maxEvaluations){
			if(evaluations % (populationSize*10) == 0) {
				...
				if(gd_.generationalDistance(previousPopulation_, population, numObjs) <= 0.5*numObjs && numGroups < numVars/5) {
					numGroups++;
					savePreviousPopulation();
					double[][][] weight = DAE.baseAE.getWeight();
					double[] W = new double[numVars];
	...
	public void reproduction() throws JMException, ClassNotFoundException {
		...
		for (int i = 0; i < (populationSize/2); i++) {
			...
			// obtain parents
			parents[0] = population.get(PseudoRandom.randInt(0, populationSize - 1));
			parents[1] = population.get(PseudoRandom.randInt(0, populationSize - 1));
			if(rd < 0.05) {//search in the original n-dimensional search space
			...
			}else if(rd < 0.95) {//search in the K-dimensional representation space
