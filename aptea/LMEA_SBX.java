...
public class LMEA_SBX extends Algorithm{
	int populationSize;
	int maxEvaluations;
	int evaluations;
	 ...
	 *            Problem to solve
	 */
	public LMEA_SBX(Problem problem) {
		super(problem);
	} // LMOEA
	...
	public void initialization() throws JMException, ClassNotFoundException {
		...
		DAE.getTrainingModel(st[0],st[1], 100);
		...
		subDAE = new DAEVariant[numGroups];
		for(int g=0;g<numGroups;g++) {
			subDAE[g] = new DAEVariant(group[g].length, 1, 3, 0.1);
		}
	...
	public SolutionSet execute() throws JMException, ClassNotFoundException {
		...
		while(evaluations <= maxEvaluations){
			if(evaluations % (populationSize*100) == 0) {
				...
				for(int g=0;g<numGroups;g++) {
					subDAE[g].getTrainingModel(st[0],st[1], 10, group[g]);
				}
			}
