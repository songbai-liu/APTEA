...
public class LMOEA_DE extends Algorithm{
	int populationSize;
	int maxEvaluations;
	int evaluations;
	 ...
	 *            Problem to solve
	 */
	public LMOEA_DE(Problem problem) {
		super(problem);
	} // LMOEA
	...
	public void reproduction() throws JMException, ClassNotFoundException {
		...
		for (int i = 0; i < (populationSize); i++) {
			...
			if(rd < productivity[0]) {//rd < productivity[t][0]
				...
				Solution offSpring = (Solution) crossoverOperator.execute(new Object[] {
						population.get(i), parents});
				mutationOperator.execute(offSpring);
				problem_.evaluate(offSpring);
				problem_.evaluateConstraints(offSpring);
				offSpring.setLearningType(1);
				...
				double[] newEncoder = doCrossover(encoder);
	...
	public double[][] encodeWithSubDAE(Solution[] parents) throws JMException {
		...
		double[][] encoder = new double[parents.length][numGroups];
	...
	public Solution decodeWithSubDAE(double[] encoder) throws JMException, ClassNotFoundException {
		Solution offspring;
		XReal child;
		offspring = new Solution(problem_);
		child = new XReal(offspring);
		for(int g=0;g<numGroups;g++) {
			double[] newEncode = new double[1];
			newEncode[0] = encoder[g];
	...
	public double[] doCrossover(double[][] encodedParents) throws JMException {
	...
	public void doMutation(double[] newEncoder) throws JMException { 
		double probability = 1.0/newEncoder.length;
		if(newEncoder.length == 1) {
			probability = 0.2;
		}
